...
sources <- c("featureNormalize_solution.R", "computeCostMulti_solution.R",
	"gradientDescentMulti_solution.R","normalEqn_solution.R","pinv.R")
...
open3d()
plot3d(X[,2],X[,3],y, 
       xlab= "sq-ft of room", ylab="#bedroom", zlab="price", main="Result of Gradient Descent", 
       col="blue", type="s",size=1.5)
...
# MATLAB like plane
nbcol <- 100
# rainbow color ramp from blue to red
color <- rev(rainbow(nbcol, start = 0/6, end = 4/6))
# zz matrix coerces to a vector and then is cut to a factor
zcol  <- cut(zz, nbcol)

# higher z values correspond to red. color[zcol] is implicitly coerced to a matrix in the opposite way it was coerced to a vector.
persp3d(xx, yy, zz, add=TRUE, col=color[zcol], alpha=0.6)
