...
sources <- c(sources, "featureNormalize_solution.R", "computeCostMulti_solution.R",
...
function name displays documentation for a built-in function. For example,
...
find the outline of an R function. Modify it to return a 5 x 5 identity matrix by filling in the following code:
...
<center>
$\displaystyle \theta_j:=\theta_j-\alpha\frac{1}{m}\sum_{i=1}^m (h_\theta (x^{(i)})-y^{(i)})x_j^{(i)}$	(simultaneously update $\theta_j$ for all $j$).
</center>
