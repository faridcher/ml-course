...
sources <- c( "plotDataPoints.r", "plotProgresskMeans.r","runkMeans.r",
"computeCentroids.r", "findClosestCentroids.r", "kMeansInitCentroids.r")
sources <- c(sources[1:3], paste0(substr(sources[4:6], 1, nchar(sources[4:6])-2), '-solution.r'))
...
dev.new(width=14)
op <- par(mfrow=c(1,2), mar=c(1, 1, 1, 1) + .1)
img <- list(A, X_recovered)
n <- c('original','compressed with 16 colors')
for(i in 1:2){
  cols <- rgb(img[[i]][,,1], img[[i]][,,2], img[[i]][,,3])
  # make a 128 by 128 matrix
  x <- structure(seq_along(cols), dim=dim(img[[i]])[1:2])
  image(t(apply(x,2,rev)), col=cols, axes=F)
  mtext(n[i],3)
}
par(op)
